...
export default function Sidebar({ collapsed = true, className }: SidebarProps) {
  ...
  const ITEMS = [
    {
      key: "dashboard",
      label: textResources.sidebar.dashboard,
      icon: <GridViewIcon />,
    },
    {
      key: "products",
      label: textResources.sidebar.products,
      icon: <Inventory2OutlinedIcon />,
    },
    {
      key: "orders",
      label: textResources.sidebar.orders,
      icon: <ReceiptLongIcon />,
    },
    {
      key: "reviews",
      label: textResources.sidebar.reviews,
      icon: <StarBorderIcon />,
    },
    {
      key: "stats",
      label: textResources.sidebar.stats,
      icon: <InsertChartOutlinedOutlinedIcon />,
    },
    {
      key: "messages",
      label: textResources.sidebar.messages,
    ...
    {
      key: "settings",
      label: textResources.sidebar.settings,
      icon: <SettingsOutlinedIcon />,
    },
  ];

  const url_for_items = [
    { key: "dashboard", path: textResources.dashboardUrls.dashboard },
    { key: "products", path: textResources.dashboardUrls.products },
    { key: "orders", path: textResources.dashboardUrls.orders },
  ...
  const handleNavigation = (itemKey: string) => {
    const item = url_for_items.find((i) => i.key === itemKey);
    if (item) {
      navigate(item.path);
    }
  };
          ...
          {ITEMS.map((it) => {
            ...
            return (
              <ListItem key={it.key} disablePadding sx={{ px: 0 }}>
                <ListItemButton
                  onClick={() => {
                    handleClick(it.key, it.label);
                    handleNavigation(it.key);
                  }}
                  sx={{
                    gap: isCollapsed ? 0 : 1,
                    ...
                    bgcolor: isActive
                      ...
                      : "transparent",
                    borderRadius:
                      !isCollapsed && isActive ? "0.75rem" : "0.5rem",
                    "&:hover": {
                      bgcolor: !isCollapsed
                        ? isActive
                          ? colors.lightGreen2
