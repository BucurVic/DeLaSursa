...
const CarouselSection: React.FC<CarouselSectionProps> = ({
  ...
  if (items.length === 0) {
    return (
      <Typography sx={{ color: colors.white2, fontStyle: "italic" }}>
    ...
    const fetchBundles = async () => {
      try {
        ...
        const producerBundles = producerId
          ? allBundles
          : allBundles.filter((b) =>
              b.producer.toLowerCase().includes(producerName.toLowerCase()),
            );
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      ...
      <Box sx={{ borderBottom: `1px solid ${colors.lightGreen1Transparent}` }}>
        <Tabs
          ...
          sx={{
            "& .MuiTab-root": {
              color: colors.white2,
              "&.Mui-selected": { color: colors.lightGreen1 },
            },
          }}
        >
      ...
      {tabValue === 2 && (
        <CarouselSection
          ...
          mobileVisibleItems={1}
          renderItem={(review: Review) => <ReviewCard {...review} />}
        />
      )}
