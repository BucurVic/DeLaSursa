...
const CustomTextField: React.FC<CustomTextFieldProps> = ({
                                                             label, name, value, onChange, placeholder = "", startIcon = null, type = "text", multiline = false, rows = 1, maxLength, select = false, children
                                                         }) => (
    <Box>
...
const CheckoutPage: React.FC = () => {
    ...
    const [cardDetails, setCardDetails] = useState<CardData>({ number: '', name: '', expiry: '', cvv: '' });
    ...
    const renderAddressFields = (data: AddressData, type: 'delivery' | 'billing') => (
        <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
                <CustomTextField label={tr.fields.fullName} name="fullName" value={data.fullName} onChange={(e: any) => handleAddressChange(e, type)} />
            </Grid>
            <Grid item xs={12} sm={6}>
                <CustomTextField label={tr.fields.phone} name="phone" value={data.phone} onChange={(e: any) => handleAddressChange(e, type)} placeholder="07xxxxxxxx" />
            </Grid>
            <Grid item xs={12}>
                <CustomTextField label={tr.fields.street} name="street" value={data.street} onChange={(e: any) => handleAddressChange(e, type)} />
            </Grid>
            <Grid item xs={12} sm={6}>
                <CustomTextField label={tr.fields.city} name="city" value={data.city} onChange={(e: any) => handleAddressChange(e, type)} />
            </Grid>
            <Grid item xs={6} sm={3}>
                <CustomTextField label={tr.fields.county} name="county" value={data.county} onChange={(e: any) => handleAddressChange(e, type)} />
            </Grid>
            <Grid item xs={6} sm={3}>
                <CustomTextField label={tr.fields.zip} name="zip" value={data.zip} onChange={(e: any) => handleAddressChange(e, type)} />
            </Grid>
        </Grid>
    );
    ...
    return (
        <Container maxWidth="md" sx={{ py: 4, color: colors.white1 }}>
            <Typography variant="h3" sx={{ fontWeight: 'bold', mb: 4, textAlign: 'center', color: colors.lightGreen1 }}>
                {tr.title}
            </Typography>

            <Stack spacing={3}>
                {/* 1. LIVRARE */}
                <Paper sx={{ p: 3, bgcolor: colors.darkGreen1, border: `1px solid ${colors.lightGreen1Transparent}`, borderRadius: '1rem' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
                        <LocationOnIcon sx={{ color: colors.lightGreen1 }} />
                        <Typography variant="h5" color={colors.white1}>{tr.sections.delivery}</Typography>
                    </Box>
                ...
                <Paper sx={{ p: 3, bgcolor: colors.darkGreen1, border: `1px solid ${colors.lightGreen1Transparent}`, borderRadius: '1rem' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                        ...
                        <Typography variant="h5" color={colors.white1}>{tr.sections.deliveryMethod}</Typography>
                    </Box>
                    <RadioGroup row value={deliveryMethod} onChange={(e) => setDeliveryMethod(e.target.value as any)}>
                        <FormControlLabel value="courier" control={<Radio sx={{ color: colors.lightGreen1, '&.Mui-checked': { color: colors.lightGreen1 } }} />} label={tr.deliveryMethods.courier} sx={{ color: colors.white1, mr: 4 }} />
                        <FormControlLabel value="pickup" control={<Radio sx={{ color: colors.lightGreen1, '&.Mui-checked': { color: colors.lightGreen1 } }} />} label={tr.deliveryMethods.pickup} sx={{ color: colors.white1 }} />
                    </RadioGroup>
                </Paper>
                ...
                <Paper sx={{ p: 3, bgcolor: colors.darkGreen1, border: `1px solid ${colors.lightGreen1Transparent}`, borderRadius: '1rem' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                        ...
                        <Typography variant="h6" color={colors.white1}>{tr.sections.billing}</Typography>
                    </Box>
                    <FormControlLabel
                        control={<Checkbox checked={billingSameAsDelivery} onChange={(e) => setBillingSameAsDelivery(e.target.checked)} sx={{ color: colors.lightGreen1, '&.Mui-checked': { color: colors.lightGreen1 } }} />}
                        label={tr.fields.sameAsDelivery}
                        sx={{ mb: 2, color: colors.white2 }}
                    />
                    {!billingSameAsDelivery && renderAddressFields(billingAddress, 'billing')}
                </Paper>
                ...
                <Paper sx={{ p: 3, bgcolor: colors.darkGreen1, border: `1px solid ${colors.lightGreen1Transparent}`, borderRadius: '1rem' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                        <PaymentIcon sx={{ color: colors.lightGreen1 }} />
                        <Typography variant="h5" color={colors.white1}>{tr.sections.payment}</Typography>
                    </Box>

                    <RadioGroup row value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} sx={{ mb: 3 }}>
                        <FormControlLabel value="card" control={<Radio sx={{ color: colors.lightGreen1, '&.Mui-checked': { color: colors.lightGreen1 } }} />} label={tr.paymentMethods.card} sx={{ color: colors.white1, mr: 4 }} />
                ...
                <Paper sx={{ p: 4, bgcolor: colors.darkGreen2, borderTop: `4px solid ${colors.lightGreen1}`, textAlign: 'center', borderRadius: '0 0 1rem 1rem' }}>
                    ...
                    <Box sx={{ maxWidth: '500px', mx: 'auto', mb: 3 }}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Typography color={colors.white2}>{tr.summary.subtotal}</Typography>
                            <Typography>{subtotal.toFixed(2)} RON</Typography>
                        </Box>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Typography color={colors.white2}>{tr.summary.shipping}</Typography>
                            <Typography>{shippingCost === 0 ? "Gratuit" : `${shippingCost.toFixed(2)} RON`}</Typography>
                        </Box>
                        <Divider sx={{ bgcolor: colors.lightGreen1Transparent, my: 2 }} />
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Typography variant="h5" color={colors.lightGreen1} fontWeight="bold">{tr.summary.total}</Typography>
                            <Typography variant="h5" color={colors.lightGreen1} fontWeight="bold">{total.toFixed(2)} RON</Typography>
                        </Box>
                    </Box>
