...
const LoginPage: React.FC = () => {
  ...
  const { login, isAuthenticated, authenticationError } =
  ...
  // 4. Modificat logica de redirect
  useEffect(() => {
    if (isAuthenticated) {
  ...
  useEffect(() => {
    if (authenticationError) {
      if (authenticationError.status === 401) {
        setSnackbarMessage("Email/parolă invalidă!");
      } else {
        setSnackbarMessage("S-a produs o eroare neasteptată");
      }
