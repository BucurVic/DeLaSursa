...
// --- 1. DEFINIREA TIPURILOR DE DATE FRONTEND ---
...
export default function SubscriptionPage() {
    ...
    const producerOptions = useMemo(() => {
        const uniqueProducers = Array.from(new Set(bundles.map(b => b.producer))).map(p => ({
            value: p,
            label: p
        }));
    ...
    const activeFilters = useMemo(() => {
        ...
        if (selectedProducer) list.push({ label: selectedProducer, onRemove: () => setSelectedProducer("") });
        if (selectedMinPrice) list.push({ label: `Min: ${selectedMinPrice} RON`, onRemove: () => setSelectedMinPrice("") });
        if (selectedMaxPrice) list.push({ label: `Max: ${selectedMaxPrice} RON`, onRemove: () => setSelectedMaxPrice("") });
        return list;
    ...
    const filteredBundles = useMemo(() => {
        ...
        return result;
    }, [bundles, search, selectedProducer, selectedMinPrice, selectedMaxPrice, selectedSort]);
    ...
    return (
        <Box sx={{ minHeight: "100vh", bgcolor: colors.darkGreen2 }}>
            ...
            <Typography variant="h6" align="center" pb={5} sx={{ color: colors.white2 }}>
                Descoperă coșuri cu produse proaspete, direct de la producători, gândite pentru nevoile tale.
            </Typography>

            {/* ZONA FILTRE */}
            <Box sx={{ maxWidth: "1400px", mx: "auto", px: 2 }}>
                <Card sx={{ width: "100%", p: "2rem", boxSizing: "border-box", display: "flex", flexDirection: "column", gap: 4, bgcolor: colors.darkGreen2, border: `1px solid ${colors.lightGreen1Transparent}` }}>
                    <Box sx={{ display: "flex", justifyContent: "center", width: "100%" }}>
                        <SearchBar placeholder="Caută pachete..." value={search} onChange={setSearch} />
                    </Box>

                    <Box sx={{ display: "flex", justifyContent: "center", alignItems: "flex-end", gap: 2, flexWrap: "wrap" }}>
                        <FilterSelect label="Producător" value={selectedProducer} options={producerOptions} onChange={(e) => handleDropdownChange("Producător", e)} />
                        <FilterSelect label="Preț Min" value={selectedMinPrice} options={generatedPriceOptions} onChange={(e) => handleDropdownChange("Preț Min", e)} />
                        <FilterSelect label="Preț Max" value={selectedMaxPrice} options={generatedPriceOptions} onChange={(e) => handleDropdownChange("Preț Max", e)} />
                        <FilterSelect label="Sortare" value={selectedSort} options={sortOptions} onChange={(e) => handleDropdownChange("Sortare", e)} />
                    </Box>
                </Card>
                ...
                <Box sx={{ mt: 2, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 1 }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                        {activeFilters.length > 0 && (
                            <Box sx={{ mt: 2, px: 2, display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                                <Typography variant="h6" sx={{ mr: 1, color: colors.white1 }}>Filtre active:</Typography>
                                {activeFilters.map((af) => (
                                    <FilterChip key={af.label} label={af.label} onRemove={af.onRemove} />
                                ))}
                                <Button size="small" onClick={() => { setSearch(""); setSelectedProducer(""); setSelectedMinPrice(""); setSelectedMaxPrice(""); setSelectedSort(""); }} sx={{ color: colors.lightGreen1 }}>
                                  Resetează filtrele
                                </Button>
                            </Box>
                        )}
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <IconButton onClick={() => setViewType("grid")} sx={{ bgcolor: viewType === "grid" ? colors.lightGreen1 : "transparent", color: viewType === "grid" ? colors.darkGreen2 : colors.white2, borderRadius: "0.5rem", "&:hover": { bgcolor: colors.lightGreen1, color: colors.darkGreen2 } }}>
                            <GridViewIcon />
                        </IconButton>
                        <IconButton onClick={() => setViewType("list")} sx={{ bgcolor: viewType === "list" ? colors.lightGreen1 : "transparent", color: viewType === "list" ? colors.darkGreen2 : colors.white2, borderRadius: "0.5rem", "&:hover": { bgcolor: colors.lightGreen1, color: colors.darkGreen2 } }}>
                            <ViewListIcon />
                        </IconButton>
                    </Box>
                </Box>
                ...
                <Box sx={{ mt: 4, mb: 8 }}>
                    {loading ? (
                        <Typography align="center" sx={{ color: colors.white1, mt: 4 }}>Se încarcă pachetele...</Typography>
                    ) : filteredBundles.length === 0 ? (
                        <Typography align="center" sx={{ color: colors.white2, mt: 4 }}>Nu am găsit pachete.</Typography>
                    ) : viewType === "grid" ? (
                        <Grid container spacing={4} justifyContent="center" alignItems="stretch">
                            {filteredBundles.map((bundle) => (
                                <Grid item xs={12} sm={6} md={4} lg={3} key={bundle.id} sx={{ display: 'flex', justifyContent: 'center' }}>
                                    <BundleCard
                                        {...bundle}
